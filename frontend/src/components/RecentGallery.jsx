      ...
      <div className="md:ml-8 justify-between">
        ...
        <div className="flex flex-col md:grid md:grid-cols-2 md:gap-x-3 gap-y-3">
          {response.slice(0, 4).map((photo) => (
            <div
              key={`${photo.id}`}
              className="items-center gap-x-4 p-2 border-gray-400 border-1 flex flex-row md:flex-col justify-center rounded-lg hover:bg-yellow-100 transition duration-100 ease-in-out cursor-pointer bg-yellow-50"
            >
              <img
                src={"http://localhost:8000/".concat(photo.image)}
                alt={`${photo.description} photo`}
                className="h-32 w-32 md:h-64 md:w-64 object-cover rounded-md"
              />
